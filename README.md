...
- **同生共死**：dsh 进程异常退出时显示错误与重试页面；退出应用时先尝试正常结束 dsh，超时后再强制清理整个子进程树。
- **最小化到托盘**：点击窗口关闭按钮不会退出，而是隐藏到系统托盘并在后台保持 dsh 运行（首次隐藏时气泡提示）；单击或双击托盘图标恢复窗口，托盘菜单可“显示主窗口”或“退出”。只有从托盘菜单选择退出才会结束应用并清理 dsh 子进程。
- **启动更新提醒**：默认在每次启动后同时检查桌面应用与 DSH；发现新版本时显示带背景模糊效果的应用内弹窗，用户可立即处理或稍后再说，也可在设置中关闭启动检查。
- **设置页集成**：在 dsh 原生设置窗口中提供“桌面应用”分类，可重启 DeepSeek Harness、分别或同时检查桌面应用与 DSH 更新、通过 npm 更新本地 DSH，并查看应用与运行环境版本。
- **插件重启无缝接管**：dshmarket 更新插件后点击“重启”时，外壳会拦截重启请求（不再弹出命令行窗口），先显示“正在重启”动画，再在固定端口上静默重启 dsh 并自动回到页面；桌面应用全程保持运行，鉴权 cookie 绑定 `127.0.0.1:端口`，同端口重启后依然有效。
- **图形化安装向导**：可自定义安装路径，自动创建桌面与开始菜单快捷方式。
- **可选一键装环境**：安装程序可自动通过 npm 全局安装 dsh，并可选安装插件商店 [dshmarket](https://www.npmjs.com/package/dshmarket)；默认走国内镜像，失败时回退官方 npm 源。
...
- 两项都不勾选：仅安装桌面外壳，你需要自行安装 dsh，否则应用启动时会显示错误与重试页面。
